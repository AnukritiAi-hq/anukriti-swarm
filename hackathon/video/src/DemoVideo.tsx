...
// These MUST match `absoluteStartSec` in scripts/generate-voiceover.mjs
// (absoluteStartSec * 30 = from-frames below).
//
// Total: 5400 frames = 3:00
const INTRO_START = 0;
const INTRO_DURATION = 900; // 0:00 – 0:30 (30s)

const CONSULTANT_START = 900;
const CONSULTANT_DURATION = 1800; // 0:30 – 1:30 (60s)

const EVIDENCE_START = 2700;
const EVIDENCE_DURATION = 1350; // 1:30 – 2:15 (45s)

const PRESCRIBER_START = 4050;
const PRESCRIBER_DURATION = 900; // 2:15 – 2:45 (30s)
...
export const DemoVideo: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      {/* Background music — plays for the entire video */}
      <Audio src={staticFile("audio/background.mp3")} volume={0.08} />
      ...
      <Sequence from={CONSULTANT_START} durationInFrames={CONSULTANT_DURATION}>
        <ClipScene
          ...
          accent="#60a5fa"
        />
        <Audio src={staticFile("audio/vo-consultant.mp3")} volume={1} />
      </Sequence>

      {/* Scene 3: Evidence Reviewer (1:30 – 2:15) */}
      <Sequence from={EVIDENCE_START} durationInFrames={EVIDENCE_DURATION}>
        <ClipScene
          ...
          accent="#34d399"
        />
        <Audio src={staticFile("audio/vo-evidence.mp3")} volume={1} />
      </Sequence>

      {/* Scene 4: Prescriber Agent (2:15 – 2:45) */}
      <Sequence from={PRESCRIBER_START} durationInFrames={PRESCRIBER_DURATION}>
        <ClipScene
          ...
          accent="#f472b6"
        />
        <Audio src={staticFile("audio/vo-prescriber.mp3")} volume={1} />
      </Sequence>
      ...
      {/* Subtitle overlay — timing is already absolute in subtitles.json */}
      <Subtitles />
    </AbsoluteFill>
  );
};
