...
Provider-isolated client that:
- Wraps google-generativeai SDK
- Handles retries with exponential backoff
- Validates responses against grounding constraints
...
Future: Swap to Claude/OpenAI/local models by implementing the same interface.
"""
...
class GeminiResponse:
    ...
    latency_ms: float
    grounded: bool  # Whether response references provided evidence
...
class GeminiClient:
    ...
    If GEMINI_API_KEY is not set, falls back to template-based generation
    (no API call, still produces useful output for demos).
    """

    def __init__(self, model: str = "gemini-1.5-flash", max_retries: int = 2) -> None:
        self.model = model
    ...
    def generate(
        ...
        If API is unavailable, returns a structured fallback response
        that still works for demo purposes.
        """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Estimate tokens (actual usage from API when available)
                prompt_tokens = len(prompt.split()) * 1.3
                completion_tokens = len(text.split()) * 1.3

                self.metrics.total_prompt_tokens += int(prompt_tokens)
                self.metrics.total_completion_tokens += int(completion_tokens)
    ...
    def _fallback_generate(self, prompt: str, context: dict[str, Any] | None, t0: float) -> GeminiResponse:
        ...
        # Generate useful fallback from context
    ...
    def _check_grounding(self, text: str, context: dict[str, Any] | None) -> bool:
        """Check if response references provided evidence (basic heuristic)."""
        if not context:
            return True
